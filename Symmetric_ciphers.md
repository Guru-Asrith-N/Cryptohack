...
in cryptography to convert text to numbers generally ascii is used    
```
...
we have to XOR the state and round_key    
```
state = [
    [206, 243, 61, 34],
    [171, 11, 93, 31],
...
you substitute what you get in state with the one given in s_box    
```
s_box = (
...
in the script given, we add all the functions we learnt in the problems before    
in the decrypt function, we use the added functions to decrypt the cipher    
we go in reverse for decrypting hence the mix column function is skipped in the first round    
```
N_ROUNDS = 10
